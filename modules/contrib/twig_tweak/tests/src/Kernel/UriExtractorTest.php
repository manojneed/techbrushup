 ...
 * @group twig_tweak
 */
final class UriExtractorTest extends KernelTestBase {
  ...
  public function testUriExtractor(): void {
    ...
    $url = $extractor->extractUri($node);
    self::assertNull($url);

    $url = $extractor->extractUri($node->get('title'));
    self::assertNull($url);

    $url = $extractor->extractUri($node->get('field_image')[0]);
    self::assertSame('public://image-test.png', $url);

    $url = $extractor->extractUri($node->get('field_image')[1]);
    self::assertNull($url);

    $url = $extractor->extractUri($node->get('field_image'));
    self::assertSame('public://image-test.png', $url);

    $url = $extractor->extractUri($node->get('field_image')->entity);
    self::assertSame('public://image-test.png', $url);
    ...
    $url = $extractor->extractUri($node->get('field_media')[0]);
    self::assertSame('public://image-test.gif', $url);

    $url = $extractor->extractUri($node->get('field_media')[1]);
    self::assertNull($url);

    $url = $extractor->extractUri($node->get('field_media'));
    self::assertSame('public://image-test.gif', $url);

    $url = $extractor->extractUri($node->get('field_media')->entity);
    self::assertSame('public://image-test.gif', $url);
