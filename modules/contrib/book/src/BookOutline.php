...
class BookOutline {
  ...
  public function prevLink(array $book_link) {
    ...
    if ($key == $book_link['nid']) {
      // The previous page in the book may be a child of the previous
      // visible link.
      if ($prev['depth'] == $book_link['depth']) {
        // The subtree will have only one link at the top level - get its data.
        $tree = $this->bookManager->bookSubtreeData($prev);
        $data = array_shift($tree);
        // The link of interest is the last child - iterate to find
        // the deepest one.
        while ($data['below']) {
          $data = end($data['below']);
        }
   ...
   */
  public function childrenLinks(array $book_link): array {
    $flat = $this->bookManager->bookTreeGetFlat($book_link);
